	...
	for (const arg of args) {
		if (arg.startsWith("--contrast=")) {
			...
			if (filterPresets[presetName]) {
				...
				console.warn(
					`Unknown preset: ${presetName}. Available: ${Object.keys(filterPresets).join(", ")}`,
				);
			}
		}
	}
...
function parseZedMaxSaturationArg(args: string[]): number | undefined {
	...
	if (raw < 0 || raw > 1) {
		console.warn(
			`--zed-max-saturation should be between 0 and 1, got ${raw}. Clamping.`,
		);
	}
...
function parseOptionalStringArg(
	args: string[],
	key: string,
): string | undefined {
	const arg = args.find((entry) => entry.startsWith(`${key}=`));
...
`);
	...
	const zedSaturatedBaseColor = parseOptionalStringArg(
		args,
		"--zed-saturated-base",
	);
	const zedSaturatedCount = parseOptionalNumberArg(
		args,
		"--zed-saturated-count",
	);
	const zedSaturatedHueStep = parseOptionalNumberArg(
		args,
		"--zed-saturated-hue-step",
	);
	const zedSaturatedSaturation = parseOptionalNumberArg(
		args,
		"--zed-saturated-saturation",
	);
	const zedSaturatedLightness = parseOptionalNumberArg(
		args,
		"--zed-saturated-lightness",
	);
	const zedRedMaxSaturation = parseOptionalNumberArg(
		args,
		"--zed-red-max-saturation",
	);
	...
	const zedRedMinLightness = parseOptionalNumberArg(
		args,
		"--zed-red-min-lightness",
	);
	...
	for (const config of themes) {
		...
		if (config.zedOutputPath) {
			...
			await bun.write(
				config.zedOutputPath,
				JSON.stringify(zedTheme, null, "\t"),
			);
			console.log(
				`Built: ${config.theme.name} (Zed) -> ${config.zedOutputPath}`,
			);
		}
		...
		if (config.shikiOutputPath) {
			...
			await bun.write(
				config.shikiOutputPath,
				JSON.stringify(shikiTheme, null, "\t"),
			);
			console.log(
				`Built: ${config.theme.name} (Shiki) -> ${config.shikiOutputPath}`,
			);
		}
		...
		const previewSlug = config.outputPath
			.replace("./dist/", "")
			.replace(".json", "");
...
function generatePreviewIndex(configs: ThemeConfig[]): string {
	const links = configs
      ...
      <div class="card-palette">${Object.values(c.theme.palette)
				.slice(0, 8)
				.map(
					(color) =>
						`<span class="mini-swatch" style="background:${toHex(color)}"></span>`,
				)
				.join("")}</div>
    </a>`;
		})
