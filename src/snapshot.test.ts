...
const skip = !process.env.SNAPSHOT;

const describeSnapshots = skip ? describe.skip : describe;

describeSnapshots("Zed theme snapshots", () => {
...
describeSnapshots("VSCode theme snapshots", () => {
  for (const { name, theme } of allThemes) {
    test(`${name}`, () => {
...
describeSnapshots("Cross-theme consistency", () => {
