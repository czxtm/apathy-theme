...
function analyzeZed(style: Record<string, any>): AnalysisRow[] {
  ...
  const pairs: [string, string, string][] = [
    ...
    ["active_line_number → gutter", style["editor.active_line_number"], gutterBg],

    // Tabs
    ["tab.active → tab.active_bg", style["text"] || style["editor.foreground"], style["tab.active_background"]],
    ["tab.inactive → tab.inactive_bg", style["text.muted"], style["tab.inactive_background"]],
    ...
    ["terminal.yellow → terminal.bg", style["terminal.ansi.yellow"], terminalBg],
...
function analyzeVSCode(colors: Record<string, string>): AnalysisRow[] {
  ...
  const pairs: [string, string, string][] = [
    ...
    // Activity bar
    ["activityBar.fg → activityBar.bg", colors["activityBar.foreground"], activityBarBg],
    ["activityBar.inactiveFg → activityBar.bg", colors["activityBar.inactiveForeground"], activityBarBg],

    // Tabs
    ["tab.activeFg → tab.activeBg", colors["tab.activeForeground"], tabActiveBg],
    ["tab.inactiveFg → tab.inactiveBg", colors["tab.inactiveForeground"], tabInactiveBg],

    // Status bar
    ["statusBar.fg → statusBar.bg", colors["statusBar.foreground"], statusBarBg],

    // Panel
    ["panelTitle.activeFg → panel.bg", colors["panelTitle.activeForeground"], panelBg],

    // Input
    ["input.fg → input.bg", colors["input.foreground"], inputBg],
    ["input.placeholder → input.bg", colors["input.placeholderForeground"], inputBg],

    // Button
    ["button.fg → button.bg", colors["button.foreground"], colors["button.background"]],

    // List
    ["list.activeFg → list.activeBg", colors["list.activeSelectionForeground"], colors["list.activeSelectionBackground"]],
    ["list.hoverFg → list.hoverBg", colors["list.hoverForeground"], colors["list.hoverBackground"]],

    // Badge
    ["badge.fg → badge.bg", colors["badge.foreground"], colors["badge.background"]],
  ];
...
function formatTable(rows: AnalysisRow[]): string {
  ...
  for (const row of rows) {
    ...
    const dL = row.deltaL != null ? (row.deltaL >= 0 ? "+" : "") + row.deltaL : "?";
    const dS = row.deltaS != null ? (row.deltaS >= 0 ? "+" : "") + row.deltaS : "?";
    lines.push(
...
describe("Cross-theme consistency", () => {
  test("status color contrast ranges", () => {
    const results: Record<string, { min: number; max: number; values: Record<string, number> }> = {};
    ...
    for (const { name, theme } of allThemes) {
      ...
      for (const status of ["error", "warning", "info", "success", "hint"] as const) {
        const key = `${status} fg`;
        const contrast = effectiveContrast(style[status], bg);
        if (contrast == null) continue;
        if (!results[key]) results[key] = { min: Infinity, max: -Infinity, values: {} };
  ...
  test("syntax token contrast ranges", () => {
    ...
    for (const { name, theme } of allThemes) {
      ...
      for (const [token, val] of Object.entries(syntax)) {
        ...
        if (contrast == null) continue;
        if (!results[token]) results[token] = { min: Infinity, max: -Infinity, values: {} };
