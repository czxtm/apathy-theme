...
function toHslChannels(
	input: string | ColorInstance | Color | OklchInput,
): HslChannels {
	const base =
...
function toOklchChannels(
	...
	const base = colorUtil.isString(input) ? new ColorJS(input) : input;
...
function renderOklch(channels: OklchChannels): string {
	try {
		return oklchInstance(channels).toString({
			format: "hex",
			collapse: false,
			alpha: true,
		});
...
function normalizeHexa(value: string): string {
	...
	try {
		return new ColorJS(value).toString({
			format: "hex",
			collapse: false,
			alpha: true,
		});
...
export class Color {
	...
	static fromHsl(channels: HslChannels): Color {
		const color = new ColorJS(
			...
			[
				normHue(channels.h),
				clamp(channels.s, 0, 100),
				clamp(channels.l, 0, 100),
			],
...
export class StatefulColor {
	...
	selected(): Color {
		return (
			this.states?.selected || this.value.mix(this.accent, 0.3).lighter(0.1)
		);
	}
...
export function makeColors<T extends object>(
	palette: T,
): { [K in keyof T]: Color } {
...
export function effectiveContrast(
	...
	try {
		...
		const bg = toColorInstance(background)
			.clone()
			.set({ alpha: () => 1 });
...
export function blendTowardContrast(
	...
	const bi = toColorInstance(background)
		.clone()
		.set({ alpha: () => 1 });
	let lo = 0;
	let hi = 1;
...
export function clampContrast(
	...
	try {
		...
		const bg = toColorInstance(background)
			.clone()
			.set({ alpha: () => 1 });
		const fg = toColorInstance(foreground)
			.clone()
			.set({ alpha: () => ci.alpha });
...
export function mkElementColors(
	...
	const border = borderOpts.transform
		? Color.create(base)
				.set({
					l: borderOpts.transform.l
						? borderOpts.transform.l(base.l)
						: base.l * 1.02,
					c: borderOpts.transform.c
						? borderOpts.transform.c(base.c)
						: base.c * 0.77,
					h: borderOpts.transform.h ? borderOpts.transform.h(base.h) : base.h,
				})
				.alpha(borderOpts.alpha ?? 1)
				.render()
		: clampContrast(
