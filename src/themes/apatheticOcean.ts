...
import { Color, ColorInput, oklch, toHex } from "@/core/color";
...
const fgcolors = (_bg: { l: number, c: number, h: number } = { l: BG_L, c: BG_C, h: SPINE_H }) => {
  return {
    default: SOFT_WHITE,
...
const pullToward =
  (weight: number, ref: number) =>
    (n: number) =>
      n + (ref - n) * weight;

const pullHueToward =
  (weight: number, refDeg: number) =>
    (n: number) => {
...
const mkAnchor =
  (c: Color, opts: AnchorWeights = DEFAULT_ANCHOR_WEIGHTS) =>
    (color: ColorInput) => {
      ...
      return Object.keys(patch).length ? Color.create(color).set(patch) : Color.create(color);
    };
