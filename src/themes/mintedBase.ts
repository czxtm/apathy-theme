...
export const p = {
  ...
  midnightSlightlyDark: bg(L - 0.015, C - 0.001),
...
const overlay = p.charcoal.saturate().lighter().transparent(0.1).rotate(15).hexa();
...
export const mintedBaseComponentOverrides: ComponentOverrides<ColorLike> = {
  editor: {
    ...
    findMatchBackground: mix(p.midnight, oklch(0.577, 0.097, 256).alpha(0.33), 0.5),
  },
  editorGutter: {
...
export const mintedBaseModifiers: NonNullable<ThemeDefinition["modifiers"]> = {
  ...
  [SemanticTokenModifier.async]: {
    transform: (color: string) => new Color(color).mix(p.lavender.alpha(0.81), 0.1),
  },
