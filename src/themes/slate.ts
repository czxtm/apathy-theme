...
const _SELECTION_BACKGROUND = mix(FOREGROUND, BACKGROUND, 0.5);
const SURFACE = colors.midnight.lighter(0.6);
const OVERLAY = mix(lighten(rawColors.midnight, 0.6), rawColors.peach, 0.05);
const _BORDER = colors.sageGray.mix(colors.midnight, 0.5);
const _ACCENT = colors.peach;
const _CARD = colors.midnight.lighter(0.15);
const _CARD_FOREGROUND = colors.mist.mix(colors.midnight, 0.2);
const _SURFACE_FOREGROUND = colors.mist.mix(colors.midnight, 0.2);
const _SURFACE_BORDER = colors.charcoal.mix(colors.midnight, 0.8);
const _SURFACE_BORDER_ACTIVE = colors.peach
	.desaturate(0.1)
	.darker(0.4)
	.transparent(0.4);
const _SURFACE_BORDER_SUBTLE = colors.charcoal.mix(colors.midnight, 0.3);
const _SURFACE_BORDER_SEPARATOR = colors.mist.alpha(0.05).hexa();
...
const status: UserInterface<ColorLike>["status"] = {
	error: mkElementColors(rawColors.crimson, {
		background: BACKGROUND,
		foreground: FOREGROUND,
	}),
	warning: mkElementColors(rawColors.gold, {
		background: BACKGROUND,
		foreground: FOREGROUND,
	}),
	info: mkElementColors(rawColors.cyan, {
		background: BACKGROUND,
		foreground: FOREGROUND,
	}),
	success: mkElementColors(rawColors.celery, {
		background: BACKGROUND,
		foreground: FOREGROUND,
	}),
};
...
const components = {
	editor: {
		...
		inactiveSelectionBackground: transparentize(
			mix(foregrounds.accent, rawColors.midnight, 0.9),
			0.7,
		),
		selectionBackground: l10(rawColors.midnight),
		selectionHighlightBackground: transparentize(
			mix(foregrounds.accent, rawColors.midnight, 0.95),
			0.5,
		),
		findRangeHighlightBackground: transparentize(
			mix(rawColors.grape, rawColors.midnight, 0.2),
			0.5,
		),
		findMatchHighlightBackground: transparentize(
			mix(rawColors.grape, rawColors.midnight, 0.8),
			0.5,
		),
		lineNumberActiveForeground: rawColors.mist,
		lineNumberForeground: rawColors.dark1,
		findMatchBackground: transparentize(
			mix(rawColors.grape, rawColors.midnight, 0.9),
			0.7,
		),
	...
	input: {
		...
		foreground: lighten(rawColors.mist, 0.1),
	...
	modifiers: {
		...
		[SemanticTokenModifier.async]: {
			transform: (color: string) =>
				new Color(color).mix(rawColors.lavender, 0.1),
		},
	},
	ui,
