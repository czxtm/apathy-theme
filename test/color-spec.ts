...
function buildReport(
  ...
  if (ct && (ct.l !== undefined || ct.c !== undefined || ct.h !== undefined)) {
    if (ct.l !== undefined)
      lines.push(`  ${mark(dL, ct.l)} ΔL  ${dL.toFixed(1).padStart(6)}  (max ${ct.l})`);
    if (ct.c !== undefined)
      lines.push(`  ${mark(dC, ct.c)} ΔC  ${dC.toFixed(1).padStart(6)}  (max ${ct.c})`);
    if (ct.h !== undefined)
      lines.push(`  ${mark(dH, ct.h)} ΔH  ${dH.toFixed(1).padStart(6)}° (max ${ct.h}°)`);
  } else {
    const threshold = opts.colorThreshold ?? 5;
    lines.push(`  ${mark(de, threshold)} ΔE  ${de.toFixed(1).padStart(6)}  (max ${threshold})`);
  ...
  if (checkAlpha) {
    lines.push(`  ${mark(dA, alphaThresh)} Δα  ${(dA * 100).toFixed(1).padStart(5)}%  (max ${(alphaThresh * 100).toFixed(0)}%)`);
  }
...
function assertColor(
  ...
  const report = () => buildReport(path, aLch, eLch, actualStr, expected, de, dL, dC, dH, dA, opts);
