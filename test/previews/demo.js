...
// Import statements
import React, { useEffect, useState } from "react"; // [semantic: namespace + function imports]
...
class ThemeDemo {
	...
	constructor(name) {
		// [semantic: method.declaration + parameter]
	...
	async getDemoData(param1, param2 = "default") {
		// [semantic: method.async.declaration + parameters]
		try {
			...
			const processData = (data) => {
				// [semantic: variable.readonly.declaration + function + parameter]
				return data.map((item) => item.toLowerCase()); // [semantic: parameter + method + parameter + method]
			};

			// Control flow
			if (param1 && param2) {
				// [semantic: parameter + operator + parameter]
				for (let i = 0; i < numbers.length; i++) {
					// [semantic: variable + variable + property]
...
const createTheme = function (options = {}) {
	...
	const defaultOptions = {
		// [semantic: variable.readonly.declaration]
...
// Export statement
export { createTheme, ThemeDemo }; // [semantic: class + function exports]
export default themeConfig; // [semantic: variable export]

Project.prototype.addItem = function (itemName) {
	// [semantic: variable + property + function + parameter]
...
var utilityCreate = function (numBoxes, location) {
	// [semantic: variable.declaration + function + parameters]
	...
	if (/^\s (.*) [0-9a-z]+||>?a$/.test(parts["name"]))
		// [semantic: regexp + method + variable]
