...
class UserManager {
	...
	public static getInstance(): UserManager {
		...
		if (!UserManager.instance) {
			// [semantic: class + property.static]
	...
	public getUser(id: number): User | undefined {
		// [semantic: method.declaration + parameter + type]
		return this.users.get(id); // [semantic: property + method + parameter]
	}

	private validateUser(user: User): boolean {
		// [semantic: method.private.declaration + parameter + type]
...
function createUser(
	...
	// [semantic: function.declaration + parameters + type]
	return {
...
async function fetchUserData(userId: number): Promise<User | null> {
	// [semantic: function.async.declaration + parameter + type]
	try {
...
function log(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
	// [semantic: function.declaration.decorator + parameters + type]
	const originalMethod = descriptor.value; // [semantic: variable.declaration + parameter + property]
	descriptor.value = function (...args: any[]) {
		// [semantic: parameter + property + parameter]
...
function findById<T extends { id: number }>(
	...
	// [semantic: function.declaration + typeParameter + parameters + type]
...
namespace Utils {
	...
	export function formatDate(date: Date): string {
		// [semantic: function.declaration + parameter + type]
...
export { createUser, fetchUserData, UserManager };
